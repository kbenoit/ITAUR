...
```{r}
myDfm <- dfm(corpus_subset(inaugCorpus, Year > 1980), verbose = FALSE)
lexdiv(myDfm, "R")
...
```{r}
readab <- readability(corpus_subset(inaugCorpus, Year > 1980), measure = "Flesch.Kincaid")
...
```{r, collapse = FALSE}
myCorpus <- corpus_subset(inaugCorpus, Year > 1980)
...
ntype(toLower("Yada yada yada.  (TADA.)"), remove_punct = TRUE)
...
ndoc(myDfm1)
nfeature(myDfm1)
myDfm2 <- dfm(inaugCorpus, remove = stopwords("english"), stem = TRUE, verbose = FALSE)
nfeature(myDfm2)
