...
jd <- dfm(jCorp, keptFeatures = "^#", valuetype = "regex") 
topfeatures(jd, nfeature(jd))
```


## Furthur analysis examples

Wordscores:
```{r}
require(quantedaData)
data("amicusCorpus")
refs <- docvars(amicusCorpus, 'testclass')
aUniD <- dfm(amicusCorpus, ignoredFeatures=stopwords("english"))
wm <- textmodel(aUniD, as.numeric(refs)-1.5, model='wordscores')
uniCoefs <- as.data.frame(wm@Sw)
preds <- predict(wm, aUniD)
plot(preds@textscores$textscore_raw)
```
...
require(quantedaData)
data('ukManifestos')
ukTexts <- texts(ukManifestos)
ukDfmUni <- dfm(ukTexts[70:101])
caUni <- textmodel(aUniD, model="ca")
```
